%% ======================================================================
%% Se inserta dentro de anexos.tex con:  \input{anexo_requerimientos}
%% ======================================================================
\chapter{Anexos}
...
\addlinespace
RF-17 & Permitir la comparación de tres arquitecturas convolucionales bajo un
protocolo idéntico & OE3 &
Los tres modelos se entrenan con la misma partición, semilla y presupuesto \\
\bottomrule
\end{longtable}
}
...
referencia y sobre recorte automático, para los mismos eventos & P4, OE3 &
...
Sección~\ref{sec:epicas}; historias de usuario, que expresan la necesidad desde el
...
\addlinespace
HU-09 & Como responsable del estudio, quiero comparar tres arquitecturas bajo un
protocolo idéntico, para que la elección final esté respaldada y no sea
arbitraria & EP-4 & B-19, B-20, B-22, B-23, B-24 \\
\addlinespace
...
Concentra las condiciones habilitantes y el diseño de la cadena. La tarea B-01 es
bloqueante: mientras no se resuelva la posible superposición entre conjuntos,
cualquier entrenamiento arriesga producir resultados no interpretables. La columna
...
\caption{Backlog del sprint 1. Capacidad comprometible estimada: 64~horas.}\label{tab:bl1}\\
\toprule
...
{\small
\begin{longtable}{>{\raggedright\arraybackslash}p{0.9cm}>{\raggedright\arraybackslash}p{4.5cm}>{\raggedright\arraybackslash}p{1.0cm}>{\raggedright\arraybackslash}p{1.9cm}>{\raggedright\arraybackslash}p{0.5cm}>{\raggedright\arraybackslash}p{1.2cm}}
\caption{Backlog del sprint 2. Capacidad comprometible estimada: 64~horas.}\label{tab:bl2}\\
\toprule
\textbf{ID} & \textbf{Tarea} & \textbf{Tipo} & \textbf{Traza} & \textbf{h} & \textbf{Prior.} \\
\midrule
\endfirsthead
...
Cierra OE3 y produce el informe final. Es el más extenso y concentra el trabajo de
escritura, por lo que su holgura es proporcionalmente mayor.

{\small
\begin{longtable}{>{\raggedright\arraybackslash}p{0.9cm}>{\raggedright\arraybackslash}p{4.5cm}>{\raggedright\arraybackslash}p{1.0cm}>{\raggedright\arraybackslash}p{1.9cm}>{\raggedright\arraybackslash}p{0.5cm}>{\raggedright\arraybackslash}p{1.2cm}}
\caption{Backlog del sprint 3. Capacidad comprometible estimada: 80~horas.}\label{tab:bl3}\\
\toprule
\textbf{ID} & \textbf{Tarea} & \textbf{Tipo} & \textbf{Traza} & \textbf{h} & \textbf{Prior.} \\
\midrule
\endfirsthead
...
\addlinespace
B-23 & Adaptar las tres arquitecturas convolucionales al protocolo común & Imp. &
RF-17 & 7 & Debe \\
\addlinespace
B-24 & Entrenar y evaluar las tres arquitecturas bajo partición, semilla y
presupuesto idénticos & Val. & RF-17, CA-04 & 10 & Debe \\
\addlinespace
B-25 & Ejecutar la cadena completa sobre la traza de evaluación en CPU y medir su
tiempo de procesamiento & Val. & RNF-05, CA-05 & 6 & Debería \\
\addlinespace
...
Doc. & --- & 8 & Debe \\
\midrule
\multicolumn{4}{r}{\textbf{Comprometido}} & \textbf{72} & \\
\multicolumn{4}{r}{\textbf{Holgura}} & 8 & \\
\bottomrule
\end{longtable}
}
...
capacidad total baja cerca de un 25\,\% y la holgura de 20~horas no alcanza a
...
bloquea a B-19 y B-20, porque entrenar antes de resolver la posible superposición
produciría métricas no interpretables. B-06 bloquea a B-07. B-08 bloquea a B-12, y
B-12 bloquea a B-14, porque la representación se construye sobre el recorte. B-17
bloquea a B-19 y B-20. B-23 bloquea a B-24, y B-25 bloquea a B-26.
...
Sección~\ref{sec:fuera-alcance}. No se incorporan como tareas de baja prioridad,
porque hacerlo sugeriría que podrían abordarse si sobrara tiempo, y no es el caso.

\subsection{tablas D1 - D7}
...
\textbf{D5. Arquitecturas a comparar.} Se comparan tres: la arquitectura de la
línea base publicada, que permite contraste directo con el trabajo previo; una
red con conexiones residuales, que aporta mayor profundidad efectiva; y una
arquitectura liviana, que atiende la restricción de inferencia en CPU sin
aceleración. Una sola arquitectura habría hecho imposible distinguir si el
desempeño obtenido proviene del recorte adaptativo o de la capacidad del modelo.
...
\caption{Riesgos identificados y mitigación prevista.}\label{sec:anexo-riesgos}\\
\toprule
